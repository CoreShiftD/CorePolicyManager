...
pub struct ReducerCtx<'a> {
    ...
    pub warmup: &'a mut crate::core::warmup::WarmupState,
...
pub struct WarmupReducer;

impl Reducer for WarmupReducer {
    fn handles(&self) -> &'static [ActionKind] {
        use std::sync::OnceLock;
        static HANDLES: OnceLock<Vec<ActionKind>> = OnceLock::new();
        HANDLES
            .get_or_init(|| {
                vec![
                    ActionKind::ScheduleWarmup,
                    ActionKind::CompleteWarmup,
                    ActionKind::ResolvePackage,
                    ActionKind::DiscoverPaths,
                    ActionKind::DispatchWarmupChunk,
                    ActionKind::RequestPackageName,
                    ActionKind::RequestPackageDir,
                    ActionKind::RequestPackagePaths,
                    ActionKind::HandleWarmupFailure,
                ]
            })
            .as_slice()
    }
    ...
    fn apply(&self, ctx: &mut ReducerCtx, action: &Action, effects: &mut Vec<Effect>) {
        ...
        match action {
            Action::ScheduleWarmup {
                ...
                if ws.in_flight.insert(package_name.clone()) {
                    ws.hash ^= crate::core::replay::hash_str(package_name);
                }
            }
            Action::CompleteWarmup { package_name } => {
                if ws.in_flight.remove(package_name) {
                    ws.hash ^= crate::core::replay::hash_str(package_name);
                }
            }
            Action::ResolvePackage { package_name } => {
                if let Some(old) = ws.dedup_cache.insert(package_name.clone(), clock) {
                    ws.hash ^= crate::core::replay::hash_str(package_name).wrapping_add(old);
                }
                ws.hash ^= crate::core::replay::hash_str(package_name).wrapping_add(clock);
            }
            Action::DiscoverPaths {
                package_name,
                base_dir,
            } => {
                let p = std::path::PathBuf::from(base_dir);
                let p_hash = crate::core::replay::hash_str(&p.to_string_lossy());
                if let Some(old) = ws.package_map.insert(package_name.clone(), p) {
                    ws.hash ^= crate::core::replay::hash_str(package_name)
                        .wrapping_add(crate::core::replay::hash_str(&old.to_string_lossy()));
                }
                ws.hash ^= crate::core::replay::hash_str(package_name).wrapping_add(p_hash);
            }
            Action::DispatchWarmupChunk { package_name, path } => {
                effects.push(Effect::Warmup {
                    source: package_name.clone(),
                    paths: vec![path.clone()],
                });
            }
            Action::RequestPackageName { pid } => {
                effects.push(Effect::ResolvePackageName { pid: *pid });
            }
            Action::RequestPackageDir { package_name } => {
                effects.push(Effect::DiscoverPackageDir {
            ...
            Action::RequestPackagePaths {
                ...
                effects.push(Effect::DiscoverPackagePaths {
                    package_name: package_name.clone(),
                    base_dir: base_dir.clone(),
                });
            }
            Action::HandleWarmupFailure { package_name, err } => {
                effects.push(Effect::Log {
                    owner: crate::core::WARMUP_OWNER,
                    level: crate::core::LogLevel::Error,
                    event: crate::core::LogEvent::Error {
                        id: 0,
                        err: format!("warmup failed: {}", err),
                    },
                });
                if ws.in_flight.remove(package_name) {
                    ws.hash ^= crate::core::replay::hash_str(package_name);
                }
                if let Some(old) = ws.negative_cache.insert(package_name.clone(), clock) {
                    ws.hash ^= crate::core::replay::hash_str(package_name).wrapping_add(old);
                }
                ws.hash ^= crate::core::replay::hash_str(package_name).wrapping_add(clock);
            }
