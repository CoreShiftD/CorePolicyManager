use crate::utensil::config::UtensilConfig;
use crate::utensil::learning::{learn_charge_point, read_threshold};
use coreshift_policy::ForegroundPackageProvider;
...
impl<R: CommandRunner> Runtime<R> {
    ...
    fn current_foreground_package(&mut self) -> Option<String> {
        self.corepolicy_foreground_package()
            .or_else(|| self.local_foreground_package())
    }

    fn corepolicy_foreground_package(&mut self) -> Option<String> {
        let status = coreshift_policy::daemon_request(
            coreshift_policy::COREPOLICY_DEFAULT_ABSTRACT_SOCKET,
            "STATUS",
        )
        .ok()?;
        parse_corepolicy_status_foreground(&status)
    }

    fn local_foreground_package(&mut self) -> Option<String> {
        let mut provider =
            coreshift_policy::AndroidForegroundPackageProvider::new(
                coreshift_policy::AndroidForegroundConfig::default(),
            )
            .ok()?;
        provider.current_package().ok().flatten()
    }
...
fn parse_corepolicy_status_foreground(status: &str) -> Option<String> {
...
mod tests {
    ...
    fn parses_corepolicy_status_foreground() {
        assert_eq!(
            parse_corepolicy_status_foreground("daemon=running\nforeground=com.example\n")
                .as_deref(),
            Some("com.example")
        );
        assert_eq!(
            parse_corepolicy_status_foreground("foreground=unknown\n"),
            None
        );
    }
