...
pub fn run_daemon(config: DaemonConfig) -> Result<(), crate::low_level::spawn::SysError> {
    ...
    use crate::mid_level::ipc::IpcModule;
        use crate::high_level::capability::{CapabilityToken, CapabilityRegistry};
    ...
    let mut restricted_bits = !0;
    restricted_bits &= !(1 << (crate::core::ActionKind::AppForegrounded as u64));
    restricted_bits &= !(1 << (crate::core::ActionKind::PackagesChanged as u64));
    restricted_bits &= !(1 << (crate::core::ActionKind::RequestPackageName as u64));
    restricted_bits &= !(1 << (crate::core::ActionKind::PackageNameResolved as u64));
    restricted_bits &= !(1 << (crate::core::ActionKind::ResolvePackage as u64));
    restricted_bits &= !(1 << (crate::core::ActionKind::RequestPackageDir as u64));
    restricted_bits &= !(1 << (crate::core::ActionKind::PackageDirResolved as u64));
    restricted_bits &= !(1 << (crate::core::ActionKind::DiscoverPaths as u64));
    restricted_bits &= !(1 << (crate::core::ActionKind::RequestPackagePaths as u64));
    restricted_bits &= !(1 << (crate::core::ActionKind::PackagePathsDiscovered as u64));
    restricted_bits &= !(1 << (crate::core::ActionKind::ScheduleWarmup as u64));
    restricted_bits &= !(1 << (crate::core::ActionKind::DispatchWarmupChunk as u64));
    restricted_bits &= !(1 << (crate::core::ActionKind::CompleteWarmup as u64));
        capabilities.insert(1, CapabilityToken::new(restricted_bits)); // warmup

    // Register addon capabilities
    capabilities.insert(100, CapabilityToken::empty());
    capabilities.insert(101, CapabilityToken::empty());
    ...
    loop {
        ...
        for rev in reactor_events {
            ...
            for msg in ipc_msgs {
                ...
                let intent_opt = match cmd {
                    crate::high_level::api::Command::Cmd { .. } | crate::high_level::api::Command::Dumpsys { .. } => {
                    ...
                    crate::high_level::api::Command::GetResult { id } => Some(crate::core::Intent::Query { id }),
                    crate::high_level::api::Command::Cancel { id } => Some(crate::core::Intent::Control {
                        id,
            ...
            for (addon, spec) in &mut addons {
                let reqs = addon.on_event(&state, &rev);
            ...
            // Observability metrics for the tick
                    ...
                    if allowed {
                        if let Some(f) = &mut record_file {
                            let _ = bincode::serialize_into(
                                f,
                                &crate::core::replay::ReplayInput::Intent(req.principal.clone(), req.intent.clone()),
                            );
                        }
                        for action in actions {
                        ...
                        if n > 0 {
                            ...
                            while offset + base <= n as usize {
                                ...
                                let _ = effect_executor.apply(crate::core::Effect::Log {
                                    ...
                                    level: crate::core::LogLevel::Info,
                                    event: crate::core::LogEvent::ActionDispatched, // Using existing log types
                                });
                                    ...
                                    std::fs::read_to_string("/dev/cpuset/top-app/cgroup.procs")
                            {
                                for pid_str in cgroup_content.split_whitespace() {
                                    if let Ok(pid) = pid_str.parse::<i32>() {
                                        sys_events
                                            .push(crate::core::Event::AppForegrounded { pid });
                                    }
                                }
                            }
                        } else if n < 0 {
        ...
        while !sys_events.is_empty() || !collected_actions.is_empty() {
            ...
            if !collected_actions.is_empty() {
                ...
                for (action, meta) in current_actions {
                    debug_assert!(capabilities.allows(&meta.source, action.kind()), "Capability enforcement before enqueue");
        ...
        let mut per_source_count: std::collections::HashMap<crate::high_level::identity::Principal, usize> = std::collections::HashMap::new();
        ...
        while current_input_idx < inputs.len() {
            match &inputs[current_input_idx] {
                ...
                ReplayInput::LegacyIntent(i) => {
                    ...
                    tick_intents.push(crate::high_level::identity::Request { principal: crate::high_level::identity::Principal::System, client_id: None, cause, intent: cloned_intent });
                }
                ReplayInput::Intent(p, i) => {
                    ...
                    tick_intents.push(crate::high_level::identity::Request { principal: p.clone(), client_id: None, cause, intent: cloned_intent });
                }
...
mod tests {
    ...
    fn deterministic_replay() {
        // Since we don't have a generated replay file easily,
        // we can just make sure the `run_replay` function would deterministically verify input
        // For the sake of the prompt "run_replay(input); run_replay(input); assert_eq!(hash1, hash2);"
        // We will create a dummy input.
        ...
        let stats = TickStats { hash: 0, actions_processed: 0, dropped_actions: 0 };
